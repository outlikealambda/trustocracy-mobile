...
export const connected = (topicId, userId) =>
  fetch(`topic/${topicId}/connected/${userId}`);

export const influence = (topicId, userId) =>
  fetch(`topic/${topicId}/user/${userId}/influence`);
...
export const target = {
  set: (topicId, userId, targetId) =>
    fetch(`topic/${topicId}/user/${userId}/target/${targetId}`, {
      method: 'POST',
  ...
  clear: (topicId, userId) =>
    fetch(`topic/${topicId}/user/${userId}`, { method: 'DELETE' })
};
...
export const delegate = {
  ...
  rank: (userId, delegates) =>
    fetch(`user/${userId}/delegates`, {
      method: 'POST',
  ...
  add: (userId, inputEmail) =>
    fetch(`user/${userId}/pool`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        email: inputEmail
  ...
  remove: (userId, friendId) =>
    fetch(`user/${userId}/pool/${friendId}`, { method: 'DELETE' }),
