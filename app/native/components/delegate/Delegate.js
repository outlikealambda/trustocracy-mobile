...
export class Delegate extends Component {
  ...
  fetchInactive = userId => {
    Api.delegate.getInactive(userId)
  ...
  saveActive = () => {
    ...
    Api.delegate.rank(userId, active)
  ...
  setInactiveStatus = newStatus => inactiveId => {
    const inactive = this.state.inactive.map(f => Object.assign(
      {},
      f,
      {
  ...
  activate = inactiveId => {
    ...
    Api.delegate.activate(this.state.userId, inactiveId)
      .then(result => {
  ...
  remove = inactiveId => {
    ...
    Api.delegate.remove(this.state.userId, inactiveId)
      .then(result => {
        if (result.status !== 200) {
          this.inactive.status.clear(inactiveId);
        } else {
  ...
  deleteInactive = (inactiveId, delay = 0) => {
    const deleteIt = () => this.setState({
      inactive: Arrays.removeWhere(this.state.inactive, p => p.id === inactiveId)
    });
  ...
  search = email => {
    return Api.delegate.add(this.state.userId, email)
      .then(response => {
